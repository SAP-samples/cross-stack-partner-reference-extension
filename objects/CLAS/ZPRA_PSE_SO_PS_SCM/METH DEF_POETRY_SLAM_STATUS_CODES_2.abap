  METHOD def_poetry_slam_status_codes_2.
    ...
    lo_entity_type = mo_model->create_entity_type_by_struct(
                                    iv_entity_type_name       = 'POETRY_SLAM_STATUS_CODES_2'
                                    is_structure              = VALUE tys_poetry_slam_status_codes_2( )
                                    iv_do_gen_prim_props         = abap_true
                                    iv_do_gen_prim_prop_colls    = abap_true
                                    iv_do_add_conv_to_prim_props = abap_true ).

    lo_entity_type->set_edm_name( 'PoetrySlamStatusCodes' ) ##NO_TEXT.
    lo_entity_type->create_complex_prop_for_vcs( 'VALUE_CONTROLS' ).


    lo_entity_set = lo_entity_type->create_entity_set( 'POETRY_SLAM_STATUS_CODES' ).
    lo_entity_set->set_edm_name( 'PoetrySlamStatusCodes' ) ##NO_TEXT.
    ...
    lo_navigation_property->set_target_entity_type_name( 'POETRY_SLAM_STATUS_CODES_3' ).
